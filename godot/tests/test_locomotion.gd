...
func _test_interrupts_and_stealth() -> void:
	...
	# 11h: crouch + Shift (want_sprint) = crouch-walk 30% faster, still WALK (no sprint)
	...
	_assert(absf(out_cjog["planar_speed"] - exp_light_walk * 1.3) < 0.01,
		"11h2: crouch+Shift speed == crouch-walk * 1.3",
		"got=%f exp=%f" % [out_cjog["planar_speed"], exp_light_walk * 1.3])
